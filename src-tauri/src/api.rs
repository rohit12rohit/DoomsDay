...
async fn fetch_api_response_config(
    ...
    Err("Cloud API is disabled. Please use Local Providers.".to_string())
}
...
async fn user_activity(
    ...
    // BLOCKED: Do not send activity to external server
    Ok(())
}
...
async fn report_api_error(
    ...
    // BLOCKED: Log to local console only
    println!("Local Error Report: {}", error_message);
}

// Models API Command
#[tauri::command]
pub async fn fetch_models() -> Result<Vec<Model>, String> {
    // BLOCKED: Return empty list or local defaults
    // The frontend will rely on your Custom Provider configurations instead
    Ok(Vec::new())
}
...
pub async fn get_activity(_app: AppHandle) -> Result<serde_json::Value, String> {
    // BLOCKED: Return empty local data
    Ok(serde_json::json!({
        "data": [],
        "total_tokens_used": 0
    }))
